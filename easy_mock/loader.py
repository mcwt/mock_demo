# -*- coding: utf-8 -*-
# Created by mcwT <1830mm@sina.com> on 2021/02/25.
import importlib
import os
import re
import sys
...
def get_api_yaml(path: Text, method: Text) -> Dict:
    ...
    for yd in get_all_yaml():
        match_list = re.findall("\\{.*?}", yd.get("url"))
        if match_list:
            for r in match_list:
                yd["url"] = yd.get("url").replace(r, ".*")

            if re.match(yd.get("url"), path) and yd.get("method", "GET") == method:
                return yd
        elif yd.get("url") == path and yd.get("method", "GET") == method:
            return yd
